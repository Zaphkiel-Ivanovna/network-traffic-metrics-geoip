...
metric_labels = ['src', 'dst', 'service', 'proto', 'country', 'asn', 'asorg']
...
def parse_packet(line):
    ...
    with geoip2.database.Reader('geolite2/GeoLite2-ASN.mmdb') as reader:
        try:
            ip_address = ipaddress.ip_address(extract_domain(m.group('src')))
            try:
                response = reader.asn(ip_address)
                if not labels['asn']:
                    labels['asn'] = response.autonomous_system_number
                if not labels['asorg']:
                    labels['asorg'] = response.autonomous_system_organization
    ...
    with geoip2.database.Reader('geolite2/GeoLite2-City.mmdb') as reader:
        try:
            ...
            try:
                ...
                if not labels['country']:
                    labels['country'] = response.country.iso_code
            except geoip2.errors.AddressNotFoundError as e:
                pass
        except ValueError:
            pass
...
async def stream_packets():
    p = await asyncio.create_subprocess_exec(
        'tcpdump', '-n', '-i', opts.interface, '-v', '-l', opts.filters,
